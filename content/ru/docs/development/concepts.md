...
Это позволяет:
- быстро копировать параметры в проект при запуске плагинов;
...
- файл Revit, который используется как шаблон-хранилище, из которого производится копирование параметров 
в целевые проекты для последующего использования плагинами.

Классы, отвечающие за работу с параметрами платформы имеют следующую структуру наследования (структура будет подробнее разобрана в соответствующей
[главе](#структура-классов-платформы)):
- RevitParam:
...
Также файл-шаблон можно открыть при помощи кнопки "Открыть шаблон параметров" на вкладке Admin платформы в Revit.
...
Конфигурация параметров - это набор параметров платформы, с которыми можно работать при помощи функционала платформы.

>Конфигурация по умолчанию - конфигурация с параметрами платформы, которые 
> <a href="#parameterrecordexample">прописаны</a> в коде классов ProjectParamConfig, 
> SystemParamConfig или SharedParamConfig.
 
>Загружаемая конфигурация - конфигурация с параметрами платформы, которые загружаются из стороннего файл в JSON-формате.

### Сохранение конфигурации
**Save** - метод класса конфигурации, который производит сохранение конфигурации в файл JSON-формата по указанному пути.
Конфигурацию можно изменить через плагин путем добавления/изменения параметров, либо вручную открыв файл после экспорта.
При этом после того, как были внесены изменения в конфигурацию (любым способом), необходимо отредактировать файл Revit, 
который используется как шаблон-хранилище, продублировав все изменения как в конфигурации.

### Загрузка конфигурации
**Load** - метод класса конфигурации, который производит загрузку конфигурации из файла JSON-формата по указанному пути.
При этом, если был передан некорректный путь и файл по пути не существует, тогда будет загружена конфигурация 
по-умолчанию.
 
Есть возможность настроить платформу таким образом, чтобы определенная загружаемая конфигурация загружалась всегда. 
Для этого необходимо:
1. Перейти в *%appdata%\pyRevit\Extensions\01.BIM.extension*;
2. Изменить файл startup.py, который является точкой входа платформы, указав путь к нужному файлу в JSON-формате.



## Параметры платформы
В платформе присутствует классы DocumentExtensions, ElementExtensions и т.п. благодаря которым есть возможность 
выполнять операции с параметрами платформы у соответствующих типов.
...
под названием "ADSK_Комплект чертежей" (<a href="#albumblueprints">см. раздел</a>). 
...
3. <span id="getrevitparamelement">GetRevitParamElement</span> - возвращает элемент, хранящий определенный 
пользователем параметр;
...
7. <span id="getsystemparamid">GetSystemParamId</span> - возвращает идентификатор встроенного 
параметра (BuiltInParameter);
...
> в плагинах платформы, например, в SharedParamConfig есть <span id="parameterrecordexample">запись</span>:  
> 
